...
public class TomatoAI : MonoBehaviour {
	...
	public float maxRange = 4.5f;				// how far I can be and still be able to attack the player
	...
	void Update()
	{
		...
		anim.SetFloat ("PunchWait", reloading);
	}
	...
	void Attack()
	{
		...
		if (hasFired) {
			reloading += Time.deltaTime;						// I need to take the time to properly reload
			// if I have taken the necessary time to reload
			if (reloading >= reloadTime) {
				hasFired = !hasFired;							// I am able to fire again
				reloading = 0;									// I will need to start the reloading process from scratch
			}
			// if I haven't thrown a projectile
		}else{
			playerHealth.currentPlayerHealth -= 30;
			hasFired = !hasFired;								// I have now fired and must reload
		}
