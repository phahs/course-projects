...
public class DrRottenAI : MonoBehaviour {
	...
	public float minRange = 10f;				// how close I can be and still be able to attack the player
...
//		randomItem = GetComponent<DropItem> ();
		...
		punchRange = (nav.radius * gameObject.transform.localScale.x * 0.5f) + (playerNav.radius * player.transform.localScale.x * 0.5f);
	}
...
//		anim.SetBool ("Moving", moving);
	...
	void Engage()
	{
		...
		if (distance <= punchRange || distance > minRange) {
...
//		Debug.Log (facing);
		newProjectile = (GameObject)Instantiate (projectile, gameObject.transform.position
		                                         + Vector3.up * 0.5f + facing * 3.5f, transform.localRotation);
